    ...
    <section id="soluciones" className="px-6 py-24 max-w-7xl mx-auto relative z-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 auto-rows-[280px]">
        {services.map((service, index) => {
          ...
          return (
            <div key={service.id} className={`group relative rounded-2xl overflow-hidden glass-card cursor-pointer transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_15px_40px_rgba(164,200,81,0.2)] hover:border-primary/50 ${isLarge ? 'md:col-span-2 md:row-span-2' : 'col-span-1'}`}>
              <div className="absolute inset-0 z-0">
                <div className="absolute inset-0 bg-gradient-to-t from-[#05060d] via-[#05060d]/80 to-transparent z-10"></div>
                <img src={service.imgUrl} alt={service.title} className="w-full h-full object-cover opacity-40 group-hover:opacity-70 group-hover:scale-105 transition-all duration-700 mix-blend-luminosity" />
              </div>
