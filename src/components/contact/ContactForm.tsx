      ...
      <div className="max-w-7xl mx-auto glass-card rounded-3xl p-6 md:p-12 border border-primary/20 shadow-2xl relative overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-stretch">
          ...
          <div className="flex flex-col h-full">
            <div className="mb-8">
              <h2 className="text-3xl md:text-5xl font-headline font-bold mb-4 text-white leading-tight">
                {content.formTitle}
              </h2>
            ...
            <div className="flex flex-wrap gap-8 mb-8">
              ...
              <a href="mailto:ventas@andicot.com" className="flex items-center gap-3 group">
                ...
                <div>
                  <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest">Correo Electrónico</p>
                  <p className="font-bold text-white text-sm">ventas@andicot.com</p>
                </div>
              </a>
            </div>

            <div className="flex-1 min-h-[350px] md:min-h-[450px] rounded-2xl overflow-hidden border border-primary/20 relative mb-6">
              <iframe 
                src={content.mapUrl} 
            ...
            <div className="flex items-start gap-3 text-gray-400">
              ...
              <div>
                <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Ubicación</p>
          ...
          <form onSubmit={handleSubmit} className="bg-background/80 p-6 md:p-10 rounded-2xl border border-gray-800 backdrop-blur-xl flex flex-col justify-between shadow-inner">
            <div className="space-y-6">
              <div>
                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Nombre o Empresa</label>
                <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-secondary/20 border border-gray-700 rounded-xl p-4 text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all" placeholder="Ej: Tech Solutions S.A." />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-secondary/20 border border-gray-700 rounded-xl p-4 text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all" placeholder="usuario@empresa.com" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Teléfono / WhatsApp</label>
                  <input required type="tel" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full bg-secondary/20 border border-gray-700 rounded-xl p-4 text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all" placeholder="+593 ..." />
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Requerimiento Técnico</label>
                <textarea required rows={7} value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} className="w-full bg-secondary/20 border border-gray-700 rounded-xl p-4 text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none resize-none transition-all" placeholder="Describa su proyecto o necesidad tecnológica..." />
              </div>
            </div>
            ...
            <div className="mt-8">
              ...
              <button type="submit" disabled={status.type === 'loading'} className="w-full bg-primary text-secondary font-black py-5 rounded-xl hover:bg-white hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 shadow-lg shadow-primary/20 disabled:opacity-50 uppercase tracking-widest">
